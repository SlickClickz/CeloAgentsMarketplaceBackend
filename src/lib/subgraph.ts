...
if (!SUBGRAPH_URL) {
  console.error(
    "[Subgraph] ❌ SUBGRAPH_URL_MAINNET is not set in environment — " +
...
export async function getSubgraphMeta(): Promise<{
  ...
  try {
    ...
    if (res.data?.errors?.length) {
      console.error(
        `[Subgraph] getMeta errors: ${res.data.errors.map((e) => e.message).join(", ")}`
      );
      return null;
    }
    ...
    console.error(
      `[Subgraph] getMeta failed: ${err.message}`,
      err.response?.data ?? ""
    );
    return null;
  }
...
export async function getSubgraphStats(): Promise<{
  ...
  try {
    ...
    if (res.data?.errors?.length) {
      console.error(
        `[Subgraph] getStats errors: ${res.data.errors.map((e) => e.message).join(", ")}`
      );
      return null;
    }
    ...
    console.error(
      `[Subgraph] getStats failed: ${err.message}`,
      err.response?.data ?? ""
    );
    return null;
  }
...
export async function getAllAgentsFromSubgraph(
  ...
  if (!SUBGRAPH_URL) {
    console.error(
      "[Subgraph] Cannot fetch agents — SUBGRAPH_URL_MAINNET not set"
    );
    return [];
  }
  ...
  while (hasMore) {
    try {
      ...
      if (lastId === "") {
        console.log(
          `[Subgraph] Response status: ${res.status}, ` +
      ...
      const agents = res.data?.data?.agents ?? [];
      console.log(
        `[Subgraph] Page returned ${agents.length} agents ` +
        `(lastId: "${lastId}")`
      );
      ...
      console.error(
        `[Subgraph] Pagination failed: ${err.message}`,
        err.response?.status ?? "",
...
export async function getAgentFromSubgraph(
  ...
  try {
    ...
    if (res.data?.errors?.length) {
      console.error(
        `[Subgraph] getAgent errors: ${res.data.errors.map((e) => e.message).join(", ")}`
      );
      return null;
    }

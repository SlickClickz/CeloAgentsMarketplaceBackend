...
// function computePlatformScore(totalScore: number): number {
...
//   let volumeScore = 0;
//   if (totalFeedbacks >= 50) volumeScore = 7;
//   else if (totalFeedbacks >= 20) volumeScore = 6;
//   else if (totalFeedbacks >= 10) volumeScore = 5;
//   else if (totalFeedbacks >= 5) volumeScore = 3;
//   else if (totalFeedbacks >= 1) volumeScore = 2;
...
// function computeSelfClawReputationScore(
...
//   if (validated >= 10) score += 3;
//   else if (validated >= 5) score += 2;
//   else if (validated >= 1) score += 1;

//   // Slashing penalty
//   if (slashed > 0) {
//     const slashRatio = slashed / Math.max(validated + slashed, 1);
//     if (slashRatio > 0.3) score -= 3;
//     else if (slashRatio > 0.1) score -= 1;
//   }

//   // Badge bonus — community-recognized quality
//   if (badges.includes("Trusted Expert")) score += 2;
//   else if (badges.includes("Reliable Output")) score += 1;

//   return Math.min(Math.max(score, 0), WEIGHTS.selfClawReputation);
// }

// function computeSelfClawSkillScore(skillCount: number): number {
//   // Reward agents that publish skills publicly —
//   // it signals confidence in their capabilities
//   if (skillCount >= 5) return WEIGHTS.selfClawSkills;
//   if (skillCount >= 3) return 7;
//   if (skillCount >= 1) return 4;
//   return 0;
// }
...
//       ? getSelfClawReputation(selfClawAgent.publicKey)
//       : Promise.resolve(null),
//     selfClawAgent?.agentName
//       ? getAgentSelfClawSkills(selfClawAgent.agentName)
//       : Promise.resolve([]),
//   ]);
...
//   const feedbackPoints = computeFeedbackScore(
//     totalFeedbacks,
//     avgFeedbackScore
//   );
...
//     const reputationPoints = computeSelfClawReputationScore(
//       selfClawReputation.reputationScore,
//       selfClawReputation.validated,
//       selfClawReputation.slashed,
//       selfClawReputation.badges
//     );
...
//     ...new Set(selfClawSkills.map((s) => mapSelfClawCategory(s.category))),
//   ];
...
export async function computeReputationScore(
  ...
  const [selfClawReputation, selfClawSkills] = await Promise.all([
    selfClawAgent?.publicKey
      ...
      : Promise.resolve(null),
    selfClawAgent?.publicKey
      ? getAgentSelfClawSkills(selfClawAgent.publicKey)  // ← publicKey not agentName
      : Promise.resolve([]),
  ]);
  ...
  const selfClawSkillCategories = [
    ...new Set(
      selfClawSkills.map((s) => mapSelfClawCategory(s.category))
    ),
  ];

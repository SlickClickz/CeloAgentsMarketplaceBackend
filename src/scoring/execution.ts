...
import { AgentScanActivity, AgentScanResponse } from "../types/agentscan";
...
export async function fetchExecutionData(
  agentId: string,
  network: CeloNetwork
): Promise<AgentScanActivity | null> {
  try {
    const response = await axios.get<AgentScanResponse>(
      `${env.agentScanApiUrl}/agents/${agentId}`,
      {
        headers: { "x-api-key": env.agentScanApiKey },
        params: { network },
        timeout: 10000,
      }
    );

    if (response.data.status !== "ok" || !response.data.data) return null;
    return response.data.data;
  } catch (err: any) {
    console.warn(
      `[Execution] Failed to fetch AgentScan data for ${agentId}: ${err.message}`
    );
    return null;
  }
}
...
// ─────────────────────────────────────────
function computeVolumeScore(totalTx: number): number {
  if (totalTx >= 1000) return 20;
  if (totalTx >= 500) return 15;
  if (totalTx >= 100) return 10;
  if (totalTx >= 10) return 5;
  if (totalTx >= 1) return 2;
  return 0;
}
...
// ─────────────────────────────────────────
function computeSuccessRateScore(successRate: number): number {
  if (successRate >= 0.99) return 10;
  if (successRate >= 0.95) return 8;
  if (successRate >= 0.90) return 6;
  if (successRate >= 0.80) return 4;
  if (successRate >= 0.60) return 2;
  return 0;
}
...
// ─────────────────────────────────────────
function computeRecencyScore(lastActiveTimestamp: string): {
  score: number;
  isInactive: boolean;
} {
  ...
  if (ageHours <= 24) return { score: 10, isInactive: false };
  if (ageHours <= 72) return { score: 7, isInactive: false };
  if (ageDays <= 7) return { score: 4, isInactive: false };
  return { score: 0, isInactive: true };
}
...
export async function computeExecutionScore(
  ...
  // Recency score
  const { score: recencyScore, isInactive } = computeRecencyScore(

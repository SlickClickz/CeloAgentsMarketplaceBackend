...
// ─────────────────────────────────────────
const ACTIVITY_CATEGORY_MAP: Record<string, SkillCategory> = {
  swap: "token-swap",
...
function normalizeActivityCategory(raw: string): SkillCategory {
...
// ─────────────────────────────────────────
function buildObservedCategorySet(
  activity: AgentScanActivity
): Set<SkillCategory> {
  const observed = new Set<SkillCategory>();
...
export function computeIntegrityScore(
  skills: AgentSkill[],
  activity: AgentScanActivity | null
): {
  score: number;
  flags: AgentFlag[];
  meta: {
  ...
  const matchedCategories = declaredCategories.filter((cat) =>
    observedCategories.includes(cat)
  );

  const unmatchedCategories = declaredCategories.filter(
    (cat) => !observedCategories.includes(cat) && cat !== "other"
  );
  ...
  if (unmatchedCategories.length > 0) {
    flags.push({
      ...
      severity:
        unmatchedCategories.length >= declaredCategories.length
          ? "critical"
          : "warning",
    });
  }

...
router.get("/:agentId", async (req, res, next) => {
    try {
        ...
        // all in one DB call
        // ───────────────────────────────────
        const agentRecord = await client_1.prisma.agent.findUnique({
            where: {
                agentId_network: {
                    agentId,
                    network: network,
        ...
        // ───────────────────────────────────
        const [scan8004Agent, selfClawAgent] = await Promise.all([
            (0, scan8004_1.getAgent)(agentId, network).catch(() => null),
            (0, selfclaw_1.getSelfClawAgentByWallet)(agentRecord.walletAddress).catch(() => null),
        ]);
        const [selfClawReputation, selfClawSkills] = await Promise.all([
            selfClawAgent?.publicKey
                ? (0, selfclaw_1.getSelfClawReputation)(selfClawAgent.publicKey).catch(() => null)
                : Promise.resolve(null),
            selfClawAgent?.publicKey
                ? (0, selfclaw_1.getAgentSelfClawSkills)(selfClawAgent.publicKey).catch(() => [])
                : Promise.resolve([]),
        ]);
        // ───────────────────────────────────
        ...
        // if DB has no skills
        // ───────────────────────────────────
        let enrichedSkills = agentRecord.skills.map((s) => ({
            name: s.name,
            description: s.description,
        ...
        if (enrichedSkills.length === 0 && scan8004Agent?.supported_protocols?.length) {
            enrichedSkills = scan8004Agent.supported_protocols.map((p) => ({
                name: p,
        ...
        const selfClawProfile = selfClawAgent
            ? {
                verified: selfClawAgent.verified,
                verificationLevel: selfClawAgent.verificationLevel ?? null,
                verificationLevelScore: (0, selfclaw_1.getVerificationLevelScore)(selfClawAgent.verificationLevel),
                agentName: selfClawAgent.agentName,
                humanId: selfClawAgent.humanId,
                pipelineComplete: selfClawAgent.pipeline
                    ? selfClawAgent.pipeline.verified &&
        ...
        res.json({
            data: {
                ...
                skills: enrichedSkills,
                skillSources: {
                    skillMd: enrichedSkills.filter((s) => s.source === "skill.md").length,
                    selfclaw: enrichedSkills.filter((s) => s.source === "selfclaw").length,
                    protocol: enrichedSkills.filter((s) => s.source === "protocol").length,
                },
                selfclaw: selfClawProfile,
                platform: platformProfile,
                chain: {
...
// GET /api/v1/agent/:agentId/score/history
// ─────────────────────────────────────────
router.get("/:agentId/score/history", async (req, res, next) => {
    try {
        const { agentId } = req.params;
        const { network, chain } = req;
        const limit = Math.min(Number(req.query.limit ?? 30), 100);
        const history = await client_1.prisma.agentScoreHistory.findMany({
            where: {
                agentId,
                network: network,
            },

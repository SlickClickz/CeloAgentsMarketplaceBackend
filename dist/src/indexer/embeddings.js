...
async function openRouterEmbedding(text) {
    const response = await axios_1.default.post("https://openrouter.ai/api/v1/embeddings", {
        model: env_1.env.openRouterEmbeddingModel,
        input: text,
    }, {
        headers: {
...
async function generateEmbedding(text) {
    ...
    try {
        ...
        if (is402) {
            console.warn("[Embeddings] OpenRouter credits exhausted — using deterministic fallback");
        }
        else if (isNetwork) {
            console.warn("[Embeddings] OpenRouter unreachable — using deterministic fallback");
        }
        else {
            console.warn(`[Embeddings] OpenRouter error: ${err.message} — using deterministic fallback`);
        }
        return deterministicEmbedding(text);
    }
}
